...
public class WaveBuilder {
	...
	readonly List<EasingCurve> curves = new List<EasingCurve>();

	readonly List<float> destinations = new List<float>();

	readonly List<float> ends = new List<float>();
	...
	public ReadOnlyCollection<EasingCurve> Curves => curves.AsReadOnly();

	/// <inheritdoc cref="Wave.CurveDestinations"/>
	/// <remarks>The given list changes as more curves are added.</remarks>
	public ReadOnlyCollection<float> CurveDestinations => destinations.AsReadOnly();

	/// <inheritdoc cref="Wave.CurveHorizontalEnds"/>
	/// <remarks>The given list changes as more curves are added.</remarks>
	public ReadOnlyCollection<float> CurveHorizontalEnds => ends.AsReadOnly();

	/// <inheritdoc cref="Wave.Width"/>
	/// <remarks>The given value changes as more curves are added.</remarks>
	public float Width => curves.Count == 0 ? 0 : ends[ends.Count - 1];

	/// <inheritdoc cref="Wave.EndValue"/>
	/// <remarks>The given value changes as more curves are added.</remarks>
	public float EndValue => curves.Count == 0 ? StartValue : destinations[destinations.Count - 1];
	...
	public WaveBuilder To(float destination) {
		...
		destinations[destinations.Count - 1] = destination;
		return this;
	}
	...
	public WaveBuilder Over(float width) {
		...
		if (curves.Count == 1) {
			...
			ends[ends.Count - 1] = ends[ends.Count - 2] + width;
		}
